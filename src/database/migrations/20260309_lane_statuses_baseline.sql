-- Baseline lane status persistence for environments missing custom status migrations.
...
-- Seed defaults for any lane that still has no status rows.
...
-- Backfill items.status_id from legacy text statuses where possible.
UPDATE items i
SET status_id = s.id
